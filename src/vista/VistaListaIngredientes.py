...
class VistaListaIngredientes(QWidget):
    ...
    def mostrar_ingredientes(self, lista_ingredientes, resultado=True):
        """
        ...
        # Mostrar mensaje si el resultado es False
        if not resultado:
            mensaje_error = QMessageBox()
            mensaje_error.setIcon(QMessageBox.Question)
            mensaje_error.setText("Error: No se pudo eliminar el ingrediente porque está asociado a una o más recetas.")
            mensaje_error.setWindowTitle("Error al eliminar ingrediente")
    ...
    def mostrar_dialogo_agregar_ingrediente(self):
        ...
        if dialogo.resultado == 1:
            self.interfaz.crear_ingrediente(dialogo.texto_nombre.text(), dialogo.texto_unidad.text(),
    ...
    def mostrar_dialogo_editar_ingrediente(self, id_ingrediente):
        ...
        dialogo.exec_()
        if dialogo.resultado == 1:
            self.interfaz.editar_ingrediente(id_ingrediente, dialogo.texto_nombre.text(), dialogo.texto_unidad.text(),
                                             dialogo.texto_valor.text(), dialogo.texto_sitioCompra.text())
            self.hide()
